# v1.4.0 release evidence and promotion gate

The v1.4.0 release uses three explicit phases. A candidate is a single immutable
commit on `release-candidate/v1.4.0`; its SHA, versions, distributions, reports,
...
2. Select `dry-run` (the default). Confirm that the latest declared, tagged, and
   PyPI versions are all v1.3.0 and that the forced minor version is v1.4.0.
...
`release-notes/v1.4.0.md`, and amends all of those files into that same commit.
The lock update is accepted only when the editable `localqueue` entry changes to
v1.4.0; any third-party resolution change fails. This campaign deliberately uses
Semantic Release's `--minor` override because the previous Python 3.10 metadata
was published in error and has no supported external consumers. A final `uv lock --check` and
...
Do not create `release-candidate/v1.4.0` by hand. If the candidate needs a fix,
...
Download `release-evidence-v1.4.0` from the run. Review
...
- `confirmation`: exactly `publish v1.4.0`.
...
SHA to `main` and `refs/tags/v1.4.0` atomically on the initial run, creates or reuses
...
After the workflow succeeds, independently confirm v1.4.0 and all 21 files on PyPI,
confirm the public/latest GitHub Release and attached evidence, and confirm `main`
and `v1.4.0` resolve to the approved SHA. Record the campaign start, candidate
