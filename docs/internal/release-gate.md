# v2.0.0 release evidence and promotion gate

The v2.0.0 release uses three explicit phases. A candidate is a single immutable
commit on `release-candidate/v2.0.0`; its SHA, versions, distributions, reports,
...
2. Select `dry-run` (the default). Confirm that the latest declared, tagged, and
   PyPI versions are all v1.3.0 and that the calculated next version is v2.0.0.
...
`release-notes/v2.0.0.md`, and amends all of those files into that same commit.
The lock update is accepted only when the editable `localqueue` entry changes to
v2.0.0; any third-party resolution change fails. A final `uv lock --check` and
...
Do not create `release-candidate/v2.0.0` by hand. If the candidate needs a fix,
...
Download `release-evidence-v2.0.0` from the run. Review
...
- `confirmation`: exactly `publish v2.0.0`.
...
SHA to `main` and `refs/tags/v2.0.0` atomically on the initial run, creates or reuses
...
After the workflow succeeds, independently confirm v2.0.0 and all 21 files on PyPI,
confirm the public/latest GitHub Release and attached evidence, and confirm `main`
and `v2.0.0` resolve to the approved SHA. Record the campaign start, candidate
